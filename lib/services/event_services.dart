  ...
  void addEvent({
    ...
    try {
      ...
      CloudinaryResponse response = await cloudinary
            .uploadFile(CloudinaryFile.fromFile(backgroundImage.path, folder: title));

        imageUrls.add(response.secureUrl);
      for (int i = 0; i < galleryImage.length; i++) {
        CloudinaryResponse res = await cloudinary
            .uploadFile(CloudinaryFile.fromFile(galleryImage[i].path, folder: title));

        imageUrls.add(res.secureUrl);
      }

      Event event = Event(title: title, backgroundImage: imageUrls[0], description: description, location: location, duration: duration, punchLine1: punchLine1, punchLine2: punchLine2, galleryImage: imageUrls, category: category,);
